...
public:
    ...
    int searchBFS(Node *parent, int element){
        ...
        if (parent->left != NULL && searchBFS(parent->left, element)){
            return 1;
        }
        if (parent->right != NULL && searchBFS(parent->right, element)){
            return 1;
        }
        return 0;
    }
...
int main(int argc, char** argv){
    ...
    if (binary->searchBFS(binary->root, convertInt(argv[1]))){
